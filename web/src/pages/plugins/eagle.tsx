    ...
    if (hydrated && !enabled) {
        return (
            <WorkspacePage grid className="library-page eagle-library-page">
                <PageHeader
                    title="Eagle 素材库"
                    description="把 Eagle 作为 S1API Studio 的外部素材来源，直接浏览和管理 Eagle 原始文件。"
                    actions={<Button icon={<ArrowLeft className="size-3.5" />} onClick={() => navigate("/assets")}>返回 S1API Studio 素材库</Button>}
                />
                <section className="mt-4 library-card-surface flex min-h-72 flex-col items-center justify-center rounded-[var(--r-xl)] px-6 py-10 text-center">
                    ...
                    <p className="mt-2 max-w-md text-sm leading-6 text-foreground/55">启用后，这里会直接显示 Eagle 原本的文件夹和文件，不会把素材复制成 S1API Studio 本地素材。</p>
        ...
        <>
            <WorkspacePage grid className="library-page assets-library-page canvas-library-page eagle-library-page">
                <div className="studio-band">
                    <PageHeader
                        title="Eagle 素材库"
                        description="Eagle 是 S1API Studio 的外部素材来源；这里复用 S1API Studio 素材库的浏览方式，直接读取和写入 Eagle 原始文件。"
                        ...
                        actions={(
                            <div className="assets-header-actions">
                                <div className="assets-header-action-buttons">
                                    ...
                                    <Button icon={<ArrowLeft className="size-3.5" />} onClick={() => navigate("/assets")}>S1API Studio 素材库</Button>
                        ...
                        <section className="min-w-0">
                            ...
                            <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
                                <div className="min-w-0">
                                    ...
                                    <p className="mt-1 text-xs text-foreground/48">{currentFolder ? "当前文件夹由 Eagle 管理，S1API Studio 只负责展示和调用。" : "当前展示 Eagle 素材库中的全部文件。"}</p>
                                </div>
                                <div className="flex flex-wrap items-center gap-2">
