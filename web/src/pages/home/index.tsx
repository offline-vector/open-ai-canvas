...
export default function IndexPage() {
    ...
    const createIndependentCanvas = () => {
        ...
        if (!user) {
            if (productConfig.guestWorkspace) {
                message.info("匿名工作区仍在恢复，请稍后再试");
                return;
            }
    ...
    return (
        <>
            <section className="app-first-project-intro border-b border-border/80 pb-8 pt-3 sm:pb-10 sm:pt-6">
                <div className="inline-flex items-center gap-2 text-xs font-semibold text-foreground/48"><WorkspaceSignalIcon variant="home" size="sm" />S1API Studio</div>
