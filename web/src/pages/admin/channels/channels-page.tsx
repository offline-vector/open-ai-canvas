    ...
    const columns: ColumnsType<ModelChannel> = [
        {
            ...
            render: (_, channel) => (
                <div>
                    ...
                    <div className="max-w-lg truncate text-xs text-foreground/45">{channel.baseUrl}</div>
                </div>
            ),
        },
        ...
        { title: "最大并发", dataIndex: "concurrencyLimit", width: 120, render: (value: number) => (value > 0 ? value : <span className="text-foreground/45">跟随系统</span>) },
        {
            ...
            width: 130,
            render: (_, channel) => (
                <Tag variant="filled" color={channel.hasApiKey ? "success" : "default"}>
                    {channel.hasApiKey ? (channel.hasSecretKey ? "AK/SK 已配置" : "API Key 已配置") : "未配置"}
                </Tag>
            ),
        },
        {
