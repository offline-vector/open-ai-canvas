...
/** Copy the previous Studio indexes into the anonymous workspace without deleting or overwriting anything. */
export async function migrateLegacyBrowserStorage() {
    ...
    const [canvas, assets] = await Promise.all([
        copyMissingLegacyEntry(storage, CANVAS_STORE_KEY, scopedStorageKey(CANVAS_STORE_KEY)),
        copyMissingLegacyEntry(storage, ASSET_STORE_KEY, scopedStorageKey(ASSET_STORE_KEY)),
    ]);
