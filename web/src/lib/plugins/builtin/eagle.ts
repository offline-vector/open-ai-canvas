...
Eagle 插件把桌面 Eagle 资料库接入 S1API Studio 素材面板。浏览器不会直接访问 Eagle：所有读取、搜索、下载和写入请求先到 S1API Studio 后端，再由后端访问管理员配置的 Eagle Local API 地址。
...
> Eagle 必须保持运行。关闭客户端、切换资料库或修改本地 API 端口后，S1API Studio 会收到连接失败，不能用空列表掩盖。

## S1API Studio 后端接口

| 操作 | S1API Studio 接口 | 用途 |
| --- | --- | --- |
...
| 原文件 | \`GET /api/plugins/eagle/items/{id}/file\` | 下载并导入 S1API Studio 资源存储 |
| 写入素材 | \`POST /api/plugins/eagle/items?baseUrl=...\` | 把图片、视频或音频写回 Eagle |
| 创建文件夹 | \`POST /api/plugins/eagle/folders?baseUrl=...\` | 在 Eagle 中创建目录 |

这些是 S1API Studio 内部登录态 API，使用 Cookie 鉴权；不要把它们当作 Eagle 官方接口直接调用。
...
导入时，后端代理下载 Eagle 原文件，前端再写入 S1API Studio 自己的图片或媒体存储。图片保留可识别的宽高和 MIME；视频、音频和其他模型文件保留字节数、扩展名、Eagle item ID、文件夹和来源元数据。

| Eagle 内容 | S1API Studio 结果 | 当前限制 |
| --- | --- | --- |
...
插件可把 S1API Studio 图片、视频或音频转换为 data URL，再调用 Eagle 添加素材。已有本地 Blob 优先直接读取；只有没有本地副本时才下载远程生成地址。默认单个手动上传文件不超过 96 MB，且 MIME 必须以 \`image/\`、\`video/\` 或 \`audio/\` 开头。

启用“自动上传生成结果”后，生成资产可以写入配置的目标文件夹。自动写回失败必须显示为写回失败，不应把“S1API Studio 已保存”误报为“Eagle 已保存”。
