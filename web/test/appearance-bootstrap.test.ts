...
test("initial HTML uses the S1API fallback until the public appearance is resolved", async () => {
    ...
    expect(html).toContain("<title>S1API Studio</title>");
...
test("appearance management exposes a server-side reset to the built-in S1API brand", async () => {
    ...
    expect(pageSource).toContain("恢复 S1API Studio 默认");
