...
func renderOperations(info Metadata) string {
	...
	b.WriteString("| 阶段 | 方法与相对路径 | 请求类型 | Studio 行为 |\n| --- | --- | --- | --- |\n")
...
func renderParameters(parameters []Parameter) string {
	if len(parameters) == 0 {
		return "> 当前适配器没有声明可配置参数；这不表示上游没有参数，只表示 Studio 尚未建立稳定映射。"
	}
	var b strings.Builder
	b.WriteString("| Studio 字段 | 类型 | 必填 | 实际上游映射 | 说明 |\n| --- | --- | --- | --- | --- |\n")
...
func renderRuntimeContract(info Metadata) string {
	var b strings.Builder
	b.WriteString("## S1API Studio 运行时合同\n\n")
	...
	if strings.TrimSpace(info.Poll) != "" {
		...
		b.WriteString("- 轮询频率应遵守上游速率限制；媒体 URL 通常有有效期，成功后应立即进入 Studio 结果处理流程。\n")
	}
	b.WriteString("- 参考素材只接受上游可访问的 HTTP(S) URL 或带 MIME 前缀的 data URL；裸 Base64、登录后才能访问的链接和 Cookie 保护地址不可用。\n")
	b.WriteString("- 插件只保证下方“当前实现”明确列出的字段。上游新增能力不会自动获得 Studio 支持，必须补适配、解析和测试。\n")
	return strings.TrimSpace(b.String())
}
