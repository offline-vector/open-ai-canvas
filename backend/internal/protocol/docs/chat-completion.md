...
Chat Completions 是基于 `messages` 的同步或流式文本协议。Studio 默认把提示词包装成一条 `user` 消息；需要多轮上下文、工具调用或结构化输出时，通过模型扩展参数传入完整 OpenAI 字段。
...
官方服务的 Base URL 通常以 `/v1` 结尾；在 Studio 中应以渠道实际配置为准。同步请求返回单个 JSON；`stream: true` 时上游返回 SSE，Studio 文本任务流负责转发和组装事件。
