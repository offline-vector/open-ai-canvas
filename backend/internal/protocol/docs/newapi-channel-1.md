...
该插件把多模态参考素材统一包装为 `input.media[]`，把分辨率、比例、时长和水印放入 `parameters`，使用 `/v1/videos` 创建与查询异步任务。它是 Studio 适配的渠道合同，不是某一家模型厂商的官方协议。
...
- NewAPI 项目与部署文档由具体运维方提供；本页以 Studio 当前 adapter 的请求事实为准。
